...
public final class UserPassword {
  ...
  // correccion regla 10 evitar magic numbers usando constantes
  ...
  /**
   * Crea un UserPassword desde texto plano y aplica hash BCrypt.
   */
  ...
  /**
   * Crea un UserPassword desde un hash almacenado.
   */
  ...
  public boolean verifyPlain(final String plainText) {
    ...
    final String normalizedPlain =
            Objects.requireNonNull(plainText, "Plain password cannot be null")
                    .trim();

    final BCrypt.Result result =
            BCrypt.verifyer()
                    .verify(normalizedPlain.toCharArray(), value);
  ...
  private static void validateMinimumLength(final String normalizedValue) {
    ...
    if (normalizedValue.length() < MINIMUM_LENGTH) {
      throw InvalidUserPasswordException
              .becauseLengthIsTooShort(MINIMUM_LENGTH);
    }
  }
}